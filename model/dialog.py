...
from pytorch_transformers import cached_path, OpenAIGPTLMHeadModel, OpenAIGPTTokenizer, GPT2LMHeadModel, GPT2Tokenizer
...
#從utils的download_pretrained_model下載gpt預訓練壓縮檔，傳回所有檔案
...
logger.info("Get pretrained model and tokenizer")                                                                 #language modeling head
tokenizer_class, model_class = (GPT2Tokenizer, GPT2LMHeadModel) if args.model == 'gpt2' else (OpenAIGPTTokenizer, OpenAIGPTLMHeadModel)
tokenizer = tokenizer_class.from_pretrained(args.model_checkpoint) #from_pretrained 加載模型權重
model = model_class.from_pretrained(args.model_checkpoint)
model.to(args.device)
add_special_tokens_(model, tokenizer)

logger.info("Sample a personality")
#載入句子編碼完的檔案(data_cache)
